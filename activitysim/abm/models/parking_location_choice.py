...
def parking_location(
    ...
    n_skim_time_periods = np.unique(network_los.los_settings['skim_time_periods']['labels']).size
    if trips_merged_df[proposed_trip_departure_period].max() > n_skim_time_periods:
        ...
        if 'trip_period' not in trips_merged_df:
            ...
            trips_merged_df['trip_period'] = network_los.skim_time_period_label(trips_merged_df[proposed_trip_departure_period])
