...
def determine_mandatory_tour_flavors(mtf_settings, model_spec, default_flavors):
    ...
    valid_flavors = ((mandatory_tour_flavors['work'] >= 1) & (mandatory_tour_flavors['school'] >= 1))
...
def determine_non_mandatory_tour_max_extension(model_settings, extension_probs, default_max_extension=2):
    ...
    if provided_max_extension is not None:
        if provided_max_extension != max_extension:
            logger.warning("Specified non mandatory tour extension does not match extension probabilities file")
...
def determine_max_trips_per_leg(default_max_trips_per_leg=4):
    ...
    if provided_max_trips_per_leg is not None:
        if provided_max_trips_per_leg != max_trips_per_leg:
            logger.warning("Provided max number of stops on tour does not match with stop frequency alternatives file")
