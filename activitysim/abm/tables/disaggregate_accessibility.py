...
def find_nearest_accessibility_zone(choosers, accessibilities, origin_col='home_zone_id', method='skims'):
    """
    Matches choosers zone to nearest accessibility zones.
    ...
    accessibility_zones = list(set(accessibilities[origin_col]))

    # First find any choosers zones that are missing from accessibilities zones
    ...
    # Update the origin zone id column names to be merged on the same col name
    # matched_df[origin_col], matched_df["real_" + origin_col] = matched_df.nearest, matched_df[origin_col]
    #return matched_df.drop(columns='nearest')
    return matched_df
...
def initialize_disaggregate_accessibility():
    ...
    # Load the dataframes. If disaggregate_accessibilities do not exist in the pipeline, it will try loading csv of that name
    ...
    logsum_cols = [x for x in proto_accessibility_df.columns if 'logsums' in x]

    # Parse the merging parameters
    assert merging_params is not None

    # Check if already assigned!
    if all([True for x in logsum_cols if x in persons_merged_df.columns]):
        return

    # Find nearest zone (spatially) with accessibilities calculated
    ...
    right_df = proto_accessibility_df[merge_cols + logsum_cols].sort_values(nearest_cols)
    ...
    if merging_params.get('method') == 'hard':
        ...
        for col in merge_cols:
            if left_df[col].dtype is not right_df[col].dtype:
                ...
                type = np.max([left_df[col].dtype, right_df[col].dtype])
                left_df[col] = left_df[col].astype(type)
                right_df[col] = right_df[col].astype(type)
        ...
        # Xpop = left_df[left_df.nearest_accessibility_zone_id==zone][merge_cols[1:]]
        # Xproto = right_df[right_df.nearest_accessibility_zone_id == zone][merge_cols[1:]]
        Xpop, Xproto = left_df[exact_cols], right_df[exact_cols]
        y = Xproto.index
        ...
        assert not any(Xproto.duplicated())   # Ensure no duplicates, would mean we're missing a variable
        # assert all(clf.predict(Xproto) == y)  # Ensure it can at least predict on itself. If not there is a problem
        # pd.DataFrame({'y': y, 'yhat': clf.predict(Xproto)})
        accuracy = round(100*sum(clf.predict(Xproto) == y) / len(y), 2)
        print('Disaggregate accessibility merge training accuracy: {}% (<100% typically means insufficient merge-on features.)'.format(accuracy))

        # Predict the nearest person ID and pull the logsums
        matched_logsums_df = right_df.loc[clf.predict(Xpop)][logsum_cols].reset_index(drop=True)
    ...
    pipeline.replace_table('persons_merged', persons_merged_df.join(merge_df[logsum_cols]))
