...
def initialize_disaggregate_accessibility():
    ...
    accessibility_cols = [x for x in proto_accessibility_df.columns if 'accessibility' in x]

    # Parse the merging parameters
    assert merging_params is not None

    # Check if already assigned!
    if set(accessibility_cols).intersection(persons_merged_df.columns) == set(accessibility_cols):
        return
    ...
    if 'nearest_accessibility_zone_id' not in persons_merged_df.columns:
        persons_merged_df = find_nearest_accessibility_zone(persons_merged_df, proto_accessibility_df)

    # Copy home_zone_id in proto-table to match the temporary 'nearest_zone_id'
    proto_accessibility_df['nearest_accessibility_zone_id'] = proto_accessibility_df.home_zone_id
    ...
    right_df = proto_accessibility_df[merge_cols + accessibility_cols].sort_values(nearest_cols).drop_duplicates()
    ...
    if merging_params.get('method') == 'soft':
        ...
        matched_logsums_df = right_df.loc[clf.predict(x_pop)][accessibility_cols].reset_index(drop=True)
    ...
    pipeline.replace_table('persons', persons_merged_df.join(merge_df[accessibility_cols]))
