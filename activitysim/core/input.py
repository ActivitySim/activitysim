...
from activitysim.core.exceptions import MissingInputTableDefinition, ModelConfigurationError
...
def read_from_table_info(table_info: InputTable, state):
    ...
    if index_col is not None:
        if index_col in df.columns:
            ...
            raise ModelConfigurationError(f"index_col '{index_col}' not in {tablename} table!")
    ...
    if keep_columns:
        ...
        if not set(keep_columns).issubset(set(df.columns)):
            ...
            raise ModelConfigurationError(f"Required columns missing from {tablename} table")
