...
def log_hwm():
    ...
    for tag in HWM:
        hwm = HWM[tag]
        logger.info(
            "high water mark %s: %.2f timestamp: %s label: %s"
    ...
    with config.open_log_file(MEM["file_name"], "a") as log_file:
        for tag in HWM:
            hwm = HWM[tag]
            print(
                "%s high water mark %s: %.2f timestamp: %s label: %s"
                % (MEM["prefix"], tag, hwm["mark"], hwm["timestamp"], hwm["label"]),
                file=log_file,
            )
...
def trace_memory_info(event=""):
    ...
    if event:
        logger.info(
            f"trace_memory_info {event} rss: {GB(rss)}GB used: {GB(vmi.used)} GB percent: {vmi.percent}%"
        )
