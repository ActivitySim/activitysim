...
def get_segment_coefficients(model_settings, segment_name):
    ...
    if legacy:
        ...
        legacy_coeffs_file_path = config.config_file_path(
            model_settings["LEGACY_COEFFICIENTS"]
        )
...
def simple_simulate_calc_row_size(
    choosers, spec, nest_spec, skims=None, trace_label=None
):
    """
    ...
    trace_label = tracing.extend_trace_label(
        trace_label, "simple_simulate_calc_row_size"
    )
    ...
    logger.debug(
        f"{trace_label} #chunk_calc row_size hwm after {sizer.hwm_tag} {sizer.hwm}"
    )
...
def simple_simulate(
    ...
    row_size = chunk_size and simple_simulate_calc_row_size(
        choosers, spec, nest_spec, skims, trace_label
    )
...
def simple_simulate_by_chunk_id(
    ...
    """
    row_size = chunk_size and simple_simulate_calc_row_size(
        choosers, spec, nest_spec, trace_label=trace_label
    )
...
def simple_simulate_logsums_calc_row_size(
    choosers, spec, nest_spec, skims, trace_label
):
    """
    ...
    if nest_spec is None:
        logger.warning(
            "simple_simulate_logsums_rpc rows_per_chunk not validated for mnl"
...
def simple_simulate_logsums(
    choosers, spec, nest_spec, skims=None, locals_d=None, chunk_size=0, trace_label=None
):
    """
    like simple_simulate except return logsums instead of making choices
    ...
    row_size = chunk_size and simple_simulate_logsums_calc_row_size(
        choosers, spec, nest_spec, skims, trace_label
    )
