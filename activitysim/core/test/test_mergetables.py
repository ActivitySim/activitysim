...
def dfg():
    return orca.DataFrameWrapper(
        "g", pd.DataFrame({"g1": [1, 2, 3]}, index=["ga", "gb", "gc"])
    )
...
def dfh():
    return orca.DataFrameWrapper(
        "h",
        pd.DataFrame(
            {"h1": range(10, 15), "g_id": ["ga", "gb", "gc", "ga", "gb"]},
...
def test_recursive_getitem():
    ...
    assert orca._recursive_getitem({"a": {"b": {"c": {"d": {}, "e": {}}}}}, "e") == {
        "d": {},
        "e": {},
    }
...
def test_dict_value_to_pairs():
    assert sorted(
        orca._dict_value_to_pairs({"c": {"a": 1, "b": 2}}), key=lambda d: next(iter(d))
    ) == [{"a": 1}, {"b": 2}]
...
def test_next_merge():
    assert orca._next_merge({"d": {"c": {}, "b": {"a": {}}}}) == {"b": {"a": {}}}
    assert orca._next_merge({"b": {"a": {}, "z": {}}}) == {"b": {"a": {}, "z": {}}}
...
def test_merge_tables1(dfa, dfz, dfb):
    ...
    expected = pd.merge(expected, dfz.to_frame(), left_on="z_id", right_index=True)
...
def test_merge_tables2(dfa, dfz, dfb, dfc):
    ...
    expected = pd.merge(expected, dfz.to_frame(), left_on="z_id", right_index=True)
    expected = pd.merge(expected, dfc.to_frame(), left_index=True, right_on="b_id")
...
def test_merge_tables3():
    df_a = pd.DataFrame({"a": [0, 1]}, index=["a0", "a1"])
    df_b = pd.DataFrame(
        {"b": [2, 3, 4, 5, 6], "a_id": ["a0", "a1", "a1", "a0", "a1"]},
        index=["b0", "b1", "b2", "b3", "b4"],
    )
    df_c = pd.DataFrame({"c": [7, 8, 9]}, index=["c0", "c1", "c2"])
    df_d = pd.DataFrame(
        {
...
def test_merge_tables_dup_columns():
    ...
    bldg_df = pd.DataFrame({"zone_id": [2, 3], "parcel_id": [0, 1]}, index=[5, 6])
    ...
    df = orca.merge_tables(
        target="households", tables=["households", "buildings"], drop_intersection=False
    )
    ...
    df = orca.merge_tables(target="households", tables=["households", "buildings"])
