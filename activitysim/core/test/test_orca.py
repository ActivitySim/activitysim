...
def df():
    return pd.DataFrame(
        [[1, 4], [2, 5], [3, 6]], columns=["a", "b"], index=["x", "y", "z"]
    )
...
def test_table_copy(df):
    ...
    orca.add_table("test_cache_uncopied", lambda: df, cache=True, copy_col=False)
    ...
    orca.add_table("test_copied_columns", pd.DataFrame(index=df.index), copy_col=True)
    orca.add_table(
    ...
    for name in [
        ...
        "test_func_uncopied",
        "test_funcd_uncopied",
        "test_funcd_uncopied2",
        "test_funcd_uncopied3",
        "test_cache_uncopied",
        "test_cached_uncopied",
        ...
        pdt.assert_series_equal(table.to_frame()["a"], table.to_frame()["a"])
...
def test_columns_for_table():
    orca.add_column("table1", "col10", pd.Series([1, 2, 3], index=["a", "b", "c"]))
    orca.add_column("table2", "col20", pd.Series([10, 11, 12], index=["x", "y", "z"]))
...
def test_columns_and_tables(df):
    ...
    assert_frames_equal(
        ...
        pd.DataFrame({"a": [1, 2, 3], "c": [7, 8, 9]}, index=["x", "y", "z"]),
    )
    ...
    assert_frames_equal(
        ...
        pd.DataFrame({"b": [2, 2.5, 3], "d": [4.0, 5.0, 6.0]}, index=["x", "y", "z"]),
    )
...
def test_steps(df):
    ...
    pdt.assert_frame_equal(
        table.to_frame(),
        pd.DataFrame({"a": [5, 7, 9], "b": [4, 5, 6]}, index=["x", "y", "z"]),
    )
...
def test_get_broadcasts():
    ...
    assert set(orca._get_broadcasts(["a", "b", "z"]).keys()) == {("a", "b"), ("z", "b")}
    assert set(orca._get_broadcasts(["a", "b", "c"]).keys()) == {("a", "b"), ("b", "c")}
...
def test_table_func_local_cols(df):
    ...
    orca.add_column("table", "new", pd.Series(["a", "b", "c"], index=df.index))
...
def test_run_and_write_tables(df, store_name):
    ...
    orca.run(["step"], iter_vars=range(11), data_out=store_name, out_interval=3)
    ...
    with pd.HDFStore(store_name, mode="r") as store:
        for year in range(0, 11, 3):
            ...
            for x in range(year):
                pdt.assert_series_equal(store[key][year_key(x)], series_year(x))

        assert "base/table" in store

        for x in range(11):
            pdt.assert_series_equal(store["10/table"][year_key(x)], series_year(x))
...
def test_injectables_cm():
    ...
    with orca.injectables():
        assert orca._INJECTABLES == {"a": "a", "b": "b", "c": "c"}
...
def test_get_raw_column(df):
    ...
    assert isinstance(
        orca.get_raw_column("test_frame", "col_series"), orca._SeriesWrapper
    )
    assert isinstance(
        orca.get_raw_column("test_frame", "col_func"), orca._ColumnFuncWrapper
    )
