...
def stop_frequency_university_parking(
    ...
    trip_choosers.loc[
        ...
        & ~(trips_grouped.transform('shift', 1).fillna(False)),
        'park_before'] = True
    # from campus
    trip_choosers.loc[
        ~(trip_choosers['parked_at_university'])
    ...
    parked_atwork_subtour_ids = trip_choosers.loc[
        ...
        & (trip_choosers['parked_at_university']), 'tour_id']
    ...
    park_to_campus = ((trip_choosers['park_before'])
                      & (trip_choosers['park_before'].shift(-1)))

    # second duplicatd trip is parking trip if going away from campus
    park_from_campus = ((trip_choosers['park_after'])
                        & ~(trip_choosers['park_after'].shift(1)))
