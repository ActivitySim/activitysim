...
* ``inverse_cdf`` and ``eet`` both sample with replacement, so duplicated sampled alternatives
...
* ``inverse_cdf`` is the fastest method, followed by ``poisson``, with ``eet`` being the slowest.
  However, for models like location choice, most runtime comes from logsum calculations and the
  total difference between ``inverse_cdf`` and ``poisson`` sampling is usually very small.
* ``poisson`` is the current default when running with simulation method explicit error terms
  ...
  still providing improved noise reduction compared to inverse-CDF sampling.
