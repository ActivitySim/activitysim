...
for i in range(len(p_tables)):
    ...
    table_data = pd.DataFrame({'TABLE': cur_table, 'FIELD': cur_table_data_fields,
                               'DTYPE': cur_table_data_dtypes, 'CREATOR': cur_cp,
                               'NCOL': cur_table_data_ncol, 'NROW': cur_table_data_nrow})
    table_data = table_data[["TABLE", "FIELD", "DTYPE", "CREATOR", "NCOL", "NROW"]]  # reorder
