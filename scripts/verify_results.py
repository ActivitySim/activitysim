...
if process_tm1:
    ...
    tm1_trips_dist = pd.concat([tm1_trips.groupby(["dist_bin"]).count()["hh_id"] +
                                tm1_jtrips.groupby(["dist_bin"]).count()["hh_id"]], axis=1)
...
asim_trips.groupby(["dist_bin"]).count()["household_id"].to_csv("outputs/asim_trips_dist.csv", na_rep=0)
