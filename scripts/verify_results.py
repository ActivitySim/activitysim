...
tm1_taz_data_filename = "tazData.csv"
...
# read distance matrix and convert to 1D array
...
access = pd.concat([tm1_access, asim_access], axis=1)
access.to_csv("outputs/access.csv", na_rep=0)
...
asim_sch["gradeschool"] = asim_sch_no_sp["gradeschool"]
...
ws_size.to_csv("outputs/work-school-location.csv", na_rep=0)

# work county to county flows

tm1_work = pd.read_csv(tm1_work_filename)
tazs = pd.read_csv(tm1_taz_data_filename)

counties = ["", "SF", "SM", "SC", "ALA", "CC", "SOL", "NAP", "SON", "MAR"]
tazs["COUNTYNAME"] = pd.Series(counties)[tazs["COUNTY"].tolist()].tolist()
...
fp = pd.concat([tm1_fp, asim_fp], axis=1)

fp.to_csv("outputs/fp.csv", na_rep=0)
...
vot = pd.concat(
   [tm1_per.groupby(["vot_bin"]).count()["hh_id"],
    asim_per.groupby(["vot_bin"]).count()["household_id"]], axis=1)

vot.to_csv("outputs/vot.csv", na_rep=0)
...
jtf = pd.concat([tm1_jtf.groupby("jtf_choice_label").count()["hh_id"],
                asim_jtf.groupby("joint_tour_frequency").count()["household_id"]], axis=1)

jtf.to_csv("outputs/jtf.csv", na_rep=0)
...
jtour_dist = pd.concat([tm1_jtours.groupby(["dist_bin"]).count()["hh_id"],
                       asim_jtours.groupby(["dist_bin"]).count()["household_id"]], axis=1)

jtour_dist.to_csv("outputs/jtour_dist.csv", na_rep=0)
...
alts = pd.concat([alts, tm1_nmtf_sum, asim_nmtf_sum], axis=1)

alts.to_csv("outputs/nmtf.csv", na_rep=0)
...
nmtd_dist = pd.concat([tm1_tours_nm.groupby(["dist_bin"]).count()["hh_id"],
                      asim_nm_tours.groupby(["dist_bin"]).count()["household_id"]], axis=1)

nmtd_dist.to_csv("outputs/nmtd_dist.csv", na_rep=0)
...
atwork_tour_freq = pd.concat([
    tm1_work_tours.groupby(["atWork_freq_str"]).count()["hh_id"],
    asim_work_tours.groupby(["atwork_subtour_frequency"]).count()["household_id"]], axis=1)

atwork_tour_freq.to_csv("outputs/atwork_tour_freq.csv", na_rep=0)
...
atw_dist = pd.concat([tm1_tours_atw.groupby(["dist_bin"]).count()["hh_id"],
                     asim_atw_tours.groupby(["dist_bin"]).count()["household_id"]], axis=1)

atw_dist.to_csv("outputs/atwork_dist.csv", na_rep=0)
...
tm1_jtrips["orig_purpose"] = pd.Categorical(tm1_jtrips["orig_purpose"],
                                            categories=tm1_trips["orig_purpose"].cat.categories)

tm1_trip_purp = tm1_trips.groupby(["orig_purpose", "tour_category"]).count()["hh_id"]
...
asim_trips["tour_category"] = asim_tours["tour_category"].loc[asim_trips["tour_id"]].tolist()

asim_trip_purp = asim_trips.groupby(["purpose", "tour_category"]).count()["household_id"]
...
tm1_trips["distance"] = distmat[tm1_trips["orig_taz"]-1, tm1_trips["dest_taz"]-1]
tm1_jtrips["distance"] = distmat[tm1_jtrips["orig_taz"]-1, tm1_jtrips["dest_taz"]-1]
asim_trips["distance"] = distmat[asim_trips["origin"]-1, asim_trips["destination"]-1]
...
trips_dist = pd.concat([tm1_trips.groupby(["dist_bin"]).count()["hh_id"] +
                       tm1_jtrips.groupby(["dist_bin"]).count()["hh_id"],
                       asim_trips.groupby(["dist_bin"]).count()["household_id"]], axis=1)

trips_dist.to_csv("outputs/trips_dist.csv", na_rep=0)

# trip scheduling

tm1_trips_tdd = tm1_trips.groupby([
    "depart_hour"]).count()["hh_id"] + tm1_jtrips.groupby(["depart_hour"]).count()["hh_id"]

asim_trips_tdd = asim_trips.groupby(["depart"]).count()["household_id"]

trips_depart = pd.concat([tm1_trips_tdd, asim_trips_tdd], axis=1)

trips_depart.to_csv("outputs/trips_depart.csv", na_rep=0)

# trip mode share by tour purpose

tm1_trips["trip_mode_str"] = pd.Series(mode_labels)[tm1_trips["trip_mode"].tolist()].tolist()
tm1_trips["trip_mode_str"] = pd.Categorical(tm1_trips["trip_mode_str"], categories=mode_labels)

tm1_jtrips["trip_mode_str"] = pd.Series(mode_labels)[tm1_jtrips["trip_mode"].tolist()].tolist()
tm1_jtrips["trip_mode_str"] = pd.Categorical(tm1_jtrips["trip_mode_str"], categories=mode_labels)

tm1_trip_mode = tm1_trips.groupby(["trip_mode_str", "tour_category"]).count()["hh_id"]
...
asim_trips["trip_mode"] = pd.Categorical(asim_trips["trip_mode"], categories=mode_labels)

asim_trip_mode = asim_trips.groupby(["trip_mode", "tour_category"]).count()["household_id"]
