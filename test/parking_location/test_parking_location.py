...
@pytest.mark.skipif(os.path.isfile('test/parking_location/output/pipeline.h5'), reason = "no need to recreate pipeline store if already exist")
...
def prepare_module_inputs() -> None:
    ...
    accessibility_file = os.path.join('test', 'parking_location', 'data', 'accessibilities.csv')
    ...
    tm2_simulated_trip_df = pd.merge(
        ...
        tm2_simulated_tour_df[['household_id', 'person_id', 'tour_id', 'tour_purpose', 'unique_tour_id', 'start_period', 'end_period']],
        how='left',
    ...
    tm2_simulated_trip_df['trip_departure_time'] = tm2_simulated_trip_df['minutes_reference']
    tm2_simulated_trip_df['trip_arrival_time'] = tm2_simulated_trip_df['trip_departure_time']  + tm2_simulated_trip_df['TRIP_TIME']
    tm2_simulated_trip_df['next_trip_departure_time'] = tm2_simulated_trip_df.groupby(['household_id','person_id','person_num'])['trip_arrival_time'].shift(-1)

    # activity duration is start of next trip - end of current trip
    tm2_simulated_trip_df['activity_duration'] = tm2_simulated_trip_df['next_trip_departure_time'] - tm2_simulated_trip_df['trip_arrival_time']

    # set default activity duration to 5 minutes
    tm2_simulated_trip_df['activity_duration'] = np.where(tm2_simulated_trip_df['activity_duration'] > 0, tm2_simulated_trip_df['activity_duration'], 5)

    # activity duration in hours
    tm2_simulated_trip_df['activity_duration_in_hours'] = tm2_simulated_trip_df['activity_duration']/60

    tm2_simulated_trip_df.drop(['period', 'start_time', 'end_time', 'minutes_reference', 'trip_departure_time', 'trip_arrival_time', 'next_trip_departure_time', 'activity_duration'], inplace=True, axis=1)
